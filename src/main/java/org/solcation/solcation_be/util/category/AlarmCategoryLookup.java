...
public class AlarmCategoryLookup {
    ...
    public AlarmCategory get(ALARMCODE code) { return Optional.ofNullable(cache.get(code)).orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_CATEGORY)); }
    public AlarmCategory get(Long acPk) { return Optional.ofNullable(pkCache.get(acPk)).orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_CATEGORY)); }
    public List<AlarmCategory> getList() { return new ArrayList<>(pkCache.values()); }
}
